...
/*   Updated: 2020/03/06 15:13:46 by lgimenez         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_rayc_drawwalls_loop(t_st *st, int side)
{
	...
	{
		st->tex.texy = (int)st->tex.texpos & (st->tex.tex_h[0][side] - 1);
		st->tex.texpos += st->tex.texstep;
		st->draw.color = st->tex.tex_data[0][side][
			(st->tex.tex_h[0][side] * st->tex.texy) + st->tex.texx];
		if (side == 1 || side == 3)
